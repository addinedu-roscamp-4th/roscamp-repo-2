import React, { useState } from 'react';
import MenuCard from './MenuItem'; // 파일명은 같지만 내부는 MenuCard로 구현됨
import { useLanguage } from '../../context/LanguageContext';

const MenuDetailModal = ({ isOpen, onClose, children }) => {
  const { t } = useLanguage();
  
  if (!isOpen) return null;
  
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-lg max-w-xl w-full p-8">
        <div className="flex justify-end">
          <button 
            onClick={onClose}
            className="text-gray-500 hover:text-gray-700 text-3xl"
            aria-label={t('common.close')}
          >
            &times;
          </button>
        </div>
        {children}
      </div>
    </div>
  );
};

const MenuGrid = ({ menuItems, onAddToCart }) => {
  const [selectedItem, setSelectedItem] = useState(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [quantity, setQuantity] = useState(1);
  const { t, language } = useLanguage();

  const openDetail = (item) => {
    setSelectedItem(item);
    setQuantity(1);
    setIsModalOpen(true);
  };

  const closeModal = () => {
    setIsModalOpen(false);
    setSelectedItem(null);
  };

  const addToCartWithQty = (item, qty) => {
    // 필요한 정보만 추출하여 새 객체 생성
    const itemToAdd = {
      id: item.id,
      name: item.name,
      price: item.price,
      image_url: item.image_url || item.image,
      description: item.description,
      prepare_time: item.prepare_time,
      category: item.category,
      categoryId: item.categoryId,
      available: item.available
    };
    
    // 수량을 한 번에 전달하여 장바구니에 추가
    onAddToCart(itemToAdd, qty);
    closeModal();
  };

  // 언어에 따른 설명 없음 메시지
  const getNoDescriptionText = () => {
    return t('menu.noDescription');
  };

  // 언어에 따른 예상 조리시간 텍스트
  const getCookingTimeText = () => {
    return t('menu.cookingTime');
  };
  
  // 가격 포맷
  const formatPrice = (price) => {
    return new Intl.NumberFormat(
      language === 'en' ? 'en-US' : language === 'ja' ? 'ja-JP' : 'ko-KR', 
      { 
        style: 'currency', 
        currency: language === 'en' ? 'USD' : language === 'ja' ? 'JPY' : 'KRW',
        minimumFractionDigits: 0 
      }
    ).format(price);
  };
  
  // 메뉴 아이템 이름과 설명 다국어 지원
  const getLocalizedMenuText = (item) => {
    if (!item) return { name: '', description: '' };
    
    // 원본 메뉴 이름과 설명
    let name = item.name;
    let description = item.description || getNoDescriptionText();
    
    // 한국어인 경우 원본 반환
    if (language === 'ko') {
      return { name, description };
    }
    
    // 번역 데이터에서 해당 메뉴 찾기
    const menuKey = Object.keys(t('menuData')).find(key => key === item.name);
    if (menuKey) {
      name = t(`menuData.${menuKey}.name`);
      // 설명이 없는 경우 번역된 '설명 없음' 텍스트 사용, 있으면 번역된 설명 사용
      description = item.description ? t(`menuData.${menuKey}.description`) : getNoDescriptionText();
    }
    
    return { name, description };
  };

  // 메뉴가 없는 경우 메시지 표시
  if (!menuItems || menuItems.length === 0) {
    return (
      <div className="flex justify-center items-center py-8">
        <p className="text-gray-500 text-xl">{t('common.noMenuItemsInCategory')}</p>
      </div>
    );
  }

  return (
    <>
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
        {menuItems.map(item => (
          <MenuCard
            key={item.id}
            item={item}
            onTap={() => openDetail(item)}
            onAdd={onAddToCart}
          />
        ))}
      </div>

      {selectedItem && (
        <MenuDetailModal isOpen={isModalOpen} onClose={closeModal}>
          <div className="h-56 w-full flex items-center justify-center bg-gray-50">
            <img 
              src={selectedItem.image_url || selectedItem.image} 
              alt={getLocalizedMenuText(selectedItem).name} 
              className="h-full object-cover"
            />
          </div>
          <h2 className="text-2xl font-bold mt-5">{getLocalizedMenuText(selectedItem).name}</h2>
          <p className="text-lg text-gray-600 mt-3">{getLocalizedMenuText(selectedItem).description}</p>
          <p className="text-sm text-gray-500 mt-1">{getCookingTimeText()} {selectedItem.prepare_time || '?'}
            {language === 'en' ? ' ' + t('menu.minutes') : language === 'ja' ? t('menu.minutes') : t('menu.minutes')}
          </p>
          
          <div className="flex items-center justify-between mt-6">
            <span className="text-xl font-medium">
              {formatPrice(selectedItem.price)}
            </span>
            
            <div className="flex items-center">
              <button 
                onClick={() => setQuantity(Math.max(1, quantity - 1))}
                className="w-12 h-12 flex items-center justify-center border border-gray-300 rounded-l text-2xl"
                aria-label={t('menu.decreaseQuantity')}
              >
                -
              </button>
              <span className="w-16 h-12 flex items-center justify-center border-t border-b border-gray-300 text-xl">
                {quantity}
              </span>
              <button 
                onClick={() => setQuantity(quantity + 1)}
                className="w-12 h-12 flex items-center justify-center border border-gray-300 rounded-r text-2xl"
                aria-label={t('menu.increaseQuantity')}
              >
                +
              </button>
            </div>
          </div>
          
          <button
            className="mt-6 w-full bg-[#C49E69] text-white py-4 rounded-md text-xl font-medium"
            onClick={() => addToCartWithQty(selectedItem, quantity)}
          >
            {t('cart.addItem')}
          </button>
        </MenuDetailModal>
      )}
    </>
  );
};

export default MenuGrid; 