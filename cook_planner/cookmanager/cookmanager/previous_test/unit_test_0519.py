...
class PoseBroadcaster(Node):
    ...
    def execute_trajectory(self, robot_id: str, trajectory_name: str, grip_timing: int, release_timing: int, segment_duration: float, sampling_time: float, grip_state: bool):
        ...
        for idx, pose in enumerate(path):
            ...
            if other_coords:
                ...
                if is_too_close(pose, other_pose, robot_id, 450, distance_publisher=self.distance_pub):
    ...
    def cook_motion_planning(self):
        ...
        for robot_id, traj_name in [('robot48', 'grip_dish_L'), ('robotb4', 'grip_dish_R')]:
            ...
            sampling_time = 0.3
            segment_duration = 4.8
            coords_list = [MycobotCoords(x=float(p[0]), y=float(p[1]), z=float(p[2]),
                                        rx=float(p[3]), ry=float(p[4]), rz=float(p[5])) for p in raw_list]
            path, speeds = interpolate_quintic_with_timing(coords_list, sampling_time, segment_duration)
            self.trajectories[robot_id] = (path, speeds)
        
        self.synchronized_execute(sampling_time, 16, 75) # 그릇 드는 것은 동시에 움직여야 하니까 싱크
        ...
        threads = []
        threads = [
            threading.Thread(target=self.execute_trajectory, args=('robot48', 'grip_ingredient_L', 200, 5, 4.8, 0.3, True)),
            threading.Thread(target=self.execute_trajectory, args=('robotb4', 'grip_ingredient_R', 200, 5, 4.8, 0.3, True))
        ]
        ...
        threads = [ # 대충 가면서 잡게끔 할라고 25
            threading.Thread(target=self.execute_trajectory, args=('robot48', 'grip_sauce_L1', 4, 200, 5.2, 0.3, False)), 
            threading.Thread(target=self.execute_trajectory, args=('robotb4', 'grip_sauce_R2', 4, 200, 5.2, 0.3, False))
        ]
        for t in threads: t.start()
        for t in threads: t.join()
        ...
        threads = []
        threads = [ # 대충 가면서 잡게끔 할라고 25
            threading.Thread(target=self.execute_trajectory, args=('robot48', 'grip_sauce_Ls', 200, 200, 1.0, 0.5, True)), 
            threading.Thread(target=self.execute_trajectory, args=('robotb4', 'grip_sauce_Rs', 200, 200, 1.0, 0.5, True))
        ]
        for t in threads: t.start()
        for t in threads: t.join()
        ...
        threads = []
        threads = [ # 대충 가면서 잡게끔 할라고 25
            threading.Thread(target=self.execute_trajectory, args=('robot48', 'grip_sauce_L2', 200, 4.1, 5.2, 0.3, True)), 
            threading.Thread(target=self.execute_trajectory, args=('robotb4', 'grip_sauce_R2', 200, 4.1, 5.2, 0.3, True))
        ]
        for t in threads: t.start()
        for t in threads: t.join()
        ...
        self.synchronized_execute(sampling_time, 30, 60) # 그릇 드는 것은 동시에 움직여야 하니까 싱크
