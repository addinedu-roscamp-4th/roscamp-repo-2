...
model = YOLO('/home/addinedu/Desktop/Yolo_ws/yolov8s/runs/pose/train/weights/best.pt')
...
# json 파일이 저장될 경로
pose_data_json_path = './PoseData/PoseData.json'
...
for idx, video_path in enumerate(video_path_list):                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             
    ...
    while True:
        ...
        # 모델에 프레임 입력 (이미지 배열)
        results = model(frame)
        result = results[0]  # 단일 프레임 처리

        # 터미널에 프레임 번호 출력
        print(f"\n=== Frame {frame_count} ===")
        
        # ---------------------------
        # 2-1. Pose Keypoints 및 스켈레톤 그리기 + 좌표 출력 & 키 프레임 정보 + 클래스 정보 데이터 프레임에 저장
        # ---------------------------
        if result.keypoints is not None:
            # 객체별 keypoints (형태: (num_objects, num_keypoints, 2))
            keypoints_all = result.keypoints.xy.cpu().numpy()
            cls_idx_all = result.boxes.cls.cpu().numpy() if result.boxes is not None else []

            frame_key = f"frame_{frame_count}"
            pose_data_dict[frame_key] = {}
            ...
            for obj_idx, keypoints in enumerate(keypoints_all):
                ...
                for kp_idx, (x, y) in enumerate(keypoints):
                    if x > 0 and y > 0:
                        ...
                        # cls_idx에 해당하는 클래스를 추출
                        cls_idx = int(cls_idx_all[obj_idx]) if obj_idx < len(cls_idx_all) else -1
                        cls_name = result.names[cls_idx]

                         # pose_data_dict에 obj_idx가 없으면 새로 생성
                        if str(obj_idx) not in pose_data_dict:
                            pose_data_dict[frame_key][str(obj_idx)] = {}

                        # Pose Data를 dictionary에 저장
                        pose_data_dict[frame_key][str(obj_idx)][str(kp_idx)] = {
                            'kp_idx': kp_idx,
                            'kp_x': int(x),
                            'kp_y': int(y),
                            'cls_name': cls_name
    ...
    with open(pose_data_json_path, 'w') as f:
        json.dump(pose_data_dict, f, indent=4)
