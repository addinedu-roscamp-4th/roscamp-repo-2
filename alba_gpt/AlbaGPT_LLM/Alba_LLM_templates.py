...
alba_task_type_list = alba_work_type_list + ["GREETINGS", "CAMERA"]
...
def alba_task_discriminator(user_query, alba_task_type_list=alba_task_type_list):
    ...
    Returns : 
        CLEANING, SERVING, MAINTENANCE, EMERGENCY, GREETINGS, CAMERA, IDLE, none 중 하나
    """
    ...
    당신은 오너의 다음 요청을 보고, 그것이 인사(GREETINGS)인지, 작업 명령(WORK)인지 구분해야 합니다.
    ...
    3. 만약 "무엇이 보이냐", "보이는 것을 설명해달라"는 요청이 포함된다면, **CAMERA**으로 간주하세요. 이 경우 '카메라'라는 단어가 없어도 무조건 CAMERA으로 판단하세요.
...
def generate_alba_camera_on_response(user_query, chat_history, recieved_image):
    """
    alba_task_discriminator 함수로 구분된 task가 'CAMERA'인 경우 해당 알바 봇으로부터 수신되는 영상의 이미지 프레임을 토대로 상황을 해석하여 json으로 반환해주는 함수입니다.
    ...
    if recieved_image:
        ...
        [조건]
        1. pinky_id는 "X번 핑키!"에서의 X 입니다. 이때 X가 없으면 pinky_id는 비워두세요.
        2. pinky_task는 무조건 string형 "CAMERA" 입니다.
...
def save_alba_response(task, alba_response) :
    ...
    if task == "GREETINGS" :
        ...
        camera_ON_file_path = os.path.join(alba_response_dir, 'CAMERA' + '_response.json')
