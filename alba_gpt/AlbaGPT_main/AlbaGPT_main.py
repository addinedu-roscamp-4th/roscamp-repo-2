...
alba_task_type_list = ["GREETINGS", "CAMERA_ON", "MAINTENANCE"]
...
def alba_server_thread(mp_stop_event, shared_dict, manager) :
    """
...
def main_thread(shared_dict) :
    memory = ConversationBufferMemory(memory_key="chat_history", input_key="user_query")
    chat_history = memory.load_memory_variables({})["chat_history"]

    user_query_example_list = ["1번 핑키 안녕!", "오늘 기분이 어때?, 1번 핑키?", "3번 핑키 베터리가 부족해 보여"]
    #user_query_example_list = ["3번 핑키 카메라 켜봐", "1번 핑키 안녕!", "오늘 기분이 어때?, 1번 핑키?", "3번 핑키 베터리가 부족해 보여"]
    ...
    for user_query in user_query_example_list :
        ...
        if discriminated_task == "GREETINGS" :
            ...
            alba_response = AlbaGPT_function.generate_alba_camera_on_response(chat_history, shared_dict)
            AlbaGPT_function.save_alba_response("CAMERA_ON", alba_response)

        else : #MAINTENANCE
            alba_response = AlbaGPT_function.generate_alba_maintenance_response(user_query, chat_history, memory)
...
if __name__=="__main__":
    ...
    vision_thread = Process(target=alba_server_thread, args=(mp_stop_event, shared_dict, manager))
    main_process = Process(target=main_thread, args=(shared_dict, ))
