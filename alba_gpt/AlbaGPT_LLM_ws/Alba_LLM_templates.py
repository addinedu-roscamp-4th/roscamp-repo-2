...
def alba_task_discriminator(user_query, alba_task_type_list=alba_task_type_list):
    ...
    """
    task_example_csv_path = './contents/response/task_example.csv' # 유저 프롬프트에 따른 분류된 task를 정리한 csv 파일
    fields = []
    task_example_list = []
...
def generate_alba_greetings_response(user_query, chat_history, memory):
    ...
    """
    greetings_example_csv_path = './contents/response/greetings_example.csv' # greetings 상황에 대한 응답을 정리한 csv 파일
    fields = []
...
def generate_alba_work_response(user_query, chat_history, memory, work):
    ...
    """
    work_example_csv_path = './contents/response/work_example.csv' # {alba_work_type} 상황에 대한 응답을 정리한 csv 파일
    fields = []
    work_example_list = []
...
def generate_alba_camera_on_response(user_query, chat_history, recieved_image):
    ...
    """
    recieved_image_dir = '../AlbaGPT_Image'

    if not os.path.exists(recieved_image_dir) :
        os.makedirs(recieved_image_dir)

    recieved_image_path = os.path.join(recieved_image_dir, 'test.jpg')
    ...
    alba_camera_on_response = multimodal_llm.invoke(recieved_image_path)['text']
