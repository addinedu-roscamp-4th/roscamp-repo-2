...
class PoseBroadcaster(Node):
    ...
    def cook_motion_planning(self):
        ...
        self.synchronized_execute(sampling_time, 20, 93) # 그릇 드는 것은 동시에 움직여야 하니까 싱크
        ...
        threads = [ # 대충 가면서 잡게끔 할라고 25
            ...
            threading.Thread(target=self.execute_trajectory, args=('robotb4', 'grip_sauce_R2', 3.5, 200, 6.0, 0.3, False))
        ]
        for t in threads: t.start()
        for t in threads: t.join()
        ...
        self.synchronized_execute(sampling_time, 37, 77) # 그릇 드는 것은 동시에 움직여야 하니까 싱크
