...
angles_1 = [0.0]*6
angles_2 = [0.0]*6
data_dirty = False  # 데이터 전송 필요 여부

def send_data_to_server(angles_1):
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            json_data = json.dumps(payload)  # 리스트 → JSON 문자열
            s.sendall(json_data.encode('utf-8'))  # utf-8 인코딩
            print(f"[📤 전송됨] {json_data}")
    except Exception as e:
        print(f"[⚠️ TCP 오류] {e}")

def tcp_sender_loop():
    global angles_1, data_dirty
    while rclpy.ok():
        if data_dirty:
            send_data_to_server(angles_1)
            print(angles_1)
            data_dirty = False
        time.sleep(1)  # 100ms 간격
...
class MyCobotTeleop(Node):
    ...
    def angles_callback_48(self, msg):
        ...
        angles_1 = [
            float(msg.joint_1), float(msg.joint_2), float(msg.joint_3),
            float(msg.joint_4), float(msg.joint_5), float(msg.joint_6)
        ]
        data_dirty = True
...
def update_status_48():
    out = status_48["coords_48"], status_48["angles_48"]
    return out

def update_status_b4():
    out = status_b4["coords_b4"], status_b4["angles_b4"]
    return out
...
def main():
    ...
    ros_thread.start()
    tcp_thread = threading.Thread(target=tcp_sender_loop, daemon=True)
    tcp_thread.start()
